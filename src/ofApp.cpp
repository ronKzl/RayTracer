...
void ofApp::setup(){
    ...
    // green (in between shiny and plain) to the right and at the front of yellow which makes it cast a shadow on it.
    Sphere s3(glm::vec3(3, 0, 1), 2, glm::vec3(0, 1, 0),25, 0.5); 
    Sphere s4(glm::vec3(-5, -2, -3), 3, glm::vec3(0, 0, 1), 32, 1.0); //blue (shiny) at the back

    Sphere s5(glm::vec3(0, -6, 0), 4, glm::vec3(0, 1, 1),20,0.5); //cyan 1/2 shiny (only rendering half of it, useful for showing shadow and the raycasting draw order using t)
    ...
    // following code adopted from Shirley's way of calculating the image and camera position (credit to Shirely)
    ...
    auto viewportUpperLeft = cameraCenter
        - glm::vec3(0, 0, focalLength) - viewportU / 2 - viewportV / 2;
    auto firstPixelLocation = viewportUpperLeft + 0.5 * (pixelDeltaU + pixelDeltaV);
    
    renderImg.allocate(imageWidth, imageHeight, OF_IMAGE_COLOR);
    ...
    for (int y = 0; y < imageHeight; y++) { 
        for (int x = 0; x < imageWidth; x++) { 
            ...
            auto pixelCenter = firstPixelLocation + (x * pixelDeltaU) + (y * pixelDeltaV);
...
glm::vec3 ofApp::ray_color(Ray& r) {
    ...
    for (Sphere& s : spheres) {
        if (s.isHit(r, 0, closestSoFar, tempRecord)) {
            ...
            persistantRecord = tempRecord;
            
        }
    }
    // compute the color if we hit anything
    if (isAnythingHit) {
        return calcThreePointLight(persistantRecord);
    }
    // else just sky color
...
glm::vec3 ofApp::calcThreePointLight(HitRecord& record) {
    ...
    glm::vec3 V = glm::normalize(cameraCenter - record.pointColision); // viewpos - intersect point
    glm::vec3 color = lightSources[0].ambient * record.objectAlbedo; // first set color to ambient (at minimum will be that)
    ...
    for (LightSource& src : lightSources) {
        ...
        for (Sphere& s : spheres) {
            ...
            if (s.isHit(shadowRay, 0.0001f, infinity, tempRecord)) {
